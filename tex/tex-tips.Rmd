...
title: "LaTeX Hints"
author: "J. R. Minter"
date: 'Started: 2013-07-16, Last modified: 2022-01-10'
output:
  html_document:
...
1. On my win 10 box latex documents are in

    ```
    C:/Users/my-user-id/OneDrive/Documents/writing/
    ```
    ```
    Update texlive with from the command prompt with:
...
2. On my MacBook     
   ...
   **TinyTeX** latex documents are in:

   ```
   /Users/my-use-id/Library/TinyTeX/texmf-var/web2c/tlmgr.log
   
   ```
   
   Note: [TinyTex](https:yihui.org/tiny.tex) (Open in Browser)
   ...
   **Homebrew**
   ...
   brew update
   brew upgrade
   brew cleanup  (complains about ghostscript)
   ```
   
   I discovered I had left out the TeX package `charter`.
   
   (N.B. it was on my list and I missed it on th Mac)
    
    
3. A **great place to start** from [Andy Roberts tips](https://www.andy-roberts.net/writing/latex)

4. A nice help on page margins from [overleaf ](https://www.overleaf.com/learn/latex/page_size_and_margins).

5. Helpful Koma examples

    [koma-script-scrlttr2](https://tex.stackexchange.com/questions/403077/what-are-the-miminal-required-environments-for-a-koma-script-scrlttr2)
    
    How to install koma-script
...
6. From `TeX and Typography` `@TeXtip`    

   You cannot use `\\` to produce a backslash. Use $\backslash$ instead.

7. A couple of good hints for problems

    **From Dikinwah Books: Top Five Mistakes Made by New Users**

    > I first started teaching LaTeX in 1998, and these are the most common errors
    > I've seen when people start learning LaTeX:
    ...
    If you encounter any problems when you start out, go through that check
    list first.

# Lecture Notes Package & Fonts
...
It takes some work to install these fonts...
...
See [here](https://www.seehuhn.de/pages/poster.html#sec:3.0.0).
...
tocloft, grfext, cochineal
```

N.B. -`cochineal` is required to use the `lecturenotes` package.
...
tlmgr install wrapfig xargs xcolor xstring xwatermark cochineal l3kernel
...
For Texshop I needed to install
