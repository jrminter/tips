...
- **Michael Droettboom** (Continuum Analytics) - [Intro](https://www.youtube.com/watch?v=tWHiUk7PRyw) notebook from [GitHub](https://github.com/dmcdougall/strata-mpl-tutorial) - from PyData 2015 NYC. He references the tutorial by [Nicolas Rougier](http://www.labri.fr/perso/nrougier/teaching/matplotlib/) who also has the source on [GitHub](https://github.com/rougier/matplotlib-tutorial). Some of this was inspired by [Ten Simple Rules for Better Figures](http://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1003833).
...
2. Install Anaconda3 2.5.0 in `C:\Apps\Anaconda3`
...
5. Run `conda update anaconda`   (does nothing)
6. Run `conda update --all`
7. Run `conda install mysql-connector-python wheel dask`
8. Install `VisualCppBuildTools_Full.exe` to get VS-2015 C++ compiler.
9. Copy `vcbuildtoolsJM.bat` into `C:\Program Files (x86)\Microsoft Visual C++ Build Tools`. Create a launcher like `Visual C++ x64 Native Build Tools Command Prompt` in `C:\Program Files (x86)\Microsoft Visual C++ Build Tools`.
10. Run `cd %GIT_HOME%\scikit-image`. Run `clean.sh`. Then run `python setup.py install` to install v.12 of scikit-image.
12. Fix path in `startMIDB3.bat`
13. Run `cd %GIT_HOME%\OSImageAnalysis\py2\yml`
14. Run `conda env create -f py27.yml`
15. Run `cd %GIT_HOME%\hyperspy`, then `git reset --hard; git checkout master`
16. Install hyperspy from command line `(source) activate py27` and `python setup install`
