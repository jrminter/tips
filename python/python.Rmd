...
title: "Python Tips"
author: "J. R. Minter"
date: 'Started: 2013-07-16, Last modified: 2021-04-24'
output:
  html_document:
...
I installed `Anaconda-Navigator` on my two Macbooks (BigSur) and Windows 10.
The documentation claims that  `Anaconda-Navigator` will alert to updates.
and install if desired. There seems to be no way to update using `conda`.

The installation would work when I started it. If I ran updates, it
would bork the installation. I am not sure why...

It looks like (2021-04-24) that I could update with

1. `conda update conda`
2. `conda update --all`

I am cautious about doing this because the last time I borked the
installation.

I also noted that clicking on the `refresh` button seems to update 
`Anaconda-Navigator`.
...
See [here](https://docs.conda.io/en/latest/miniconda.html) for the latest
miniconda release. **Note** I have had issues on macos if I update the
install. **I no longer (2020-04-24) use miniconda**.

## Keeping Spyder in the dock

From [here](https://www.youtube.com/watch?v=rQNYNRoFacA)

We need to permit applications from all developers. 
We do this from the command line:
...
## Starting Python3 on Mac

This assumes anaconda python...

In `.bash_profile` add `alias python=3.9`
...
I often want to add or remove the `conda-forge` channel...
...
## Activating/Deactivating/deleting environments
...
From 
[here](https://stackoverflow.com/questions/41274007/anaconda-export-environment-file).
...
## A clean install of python with conda.

I prefer **a clean install** when key updates are needed
instead of accumulating many unused
packages that are not removed by `conda clean -a -y`.

This will run the `Fiji/ImageJ` tutorials using a `Groovy` kernel
in a `jupyter` notebook. This is **way cool**...
...
2. Delete the miniconda directory and all evidence
of conda:

    ```
    rm -rf minconda3 .conda .condarc
    ```
    
    Also check that conda is not in the path in
    `.bash_profile`
    
3. Install the miniconda3 environment

4. Install the desired packages. My current list is shown
below.
    ...
    conda install sqlite pandas xlwt scikit-image nbconvert pandoc
    conda config --add channels conda-forge
    conda install mahotas sympy cython astropy
    # OPTIONALLY install pkgs suggested by Curtis Rueden
    ...
    ```
    # Set up a scijava environment
    conda create -y -n scijava 'python>=3'
    source activate scijava
    conda install jupyter_contrib_nbextensions numpy openjdk=8
    conda install pyimagej python rise scikit-image
    conda install openjdk=8.0.121
    conda install notebook>=5.7.6
    conda install tornado>6
    conda install ipywidgets>=7.0.0
    conda install nodejs pandas maven
    conda install py4j requests
    ...
    ```
    cd /Users/jrminter/Documents/git/beakerx/doc/groovy
    conda activate scijava
    jupyter notebook
...
## Packages in environment at /Users/jrminter/miniconda3:
    
**2019-06-26** Current packages in base environment
    
    ```
    conda list -n base 
    ```
    
Packages are [here](./environs/base.html).
    

**2019-05-02** Current packages in scijava environment
    
    ```
    conda list -n scijava 
    ```
    
Results are [here](./environs/scijava.html).
    


# Set up to work with Fiji/ImageJ Tutorials

    a. Clone this [imagej/tutorials](https://github.com/imagej/tutorials)
       repository.
       
    b. Open a console and cd to your cloned working copy.
    ...
    c. Run:
        ...
        conda env create -f environment.yml
        ```
        to create a conda environment with the dependencies these
        notebooks need. type `source activate scijava`
        (or `activate scijava` on Windows) to activate the 
    ...
    d. Run:
        ...
        jupyter notebook
        ```
        to launch Jupyter Notebook in a web browser window.
        
    e. In the browser, click into notebooks, then click on the
       `ImageJ-Tutorials-and-Demo.ipynb` notebook to open it.
       
    f. Fix issue on Windows 10
    
       See [stackoverflow](https://stackoverflow.com/questions/16428098/groovy-shell-warning-could-not-open-create-prefs-root-node)
       regarding an issue where Win10 cannot write preferences.
       
       - Go into your Start Menu and type `regedit` into the search field.
       
       - Navigate to path `HKEY_LOCAL_MACHINE\Software\JavaSoft`
       (Windows 10 may have this here:
...
> To delete packages, caches, and other files that aren't being used in
> any of your environments run:

```
conda clean -a -y
```

# Render obstinate `.ipynb` files to PDF
...
0. Install what we need from python
    ...
    **not needed** with clean install above...
...
1. use as a command line tool
    ...
    See the
    [usage section](https://nbconvert.readthedocs.io/en/latest/usage.html).
...
# Fixing Spyder on Python 3.7.2 miniconda x64 MacOSX

The problem:

```
$ spyder
/Users/jrminter/miniconda3/bin/pythonw: line 3: /Users/jrminter/miniconda3/python.app/Contents/MacOS/python: No such file or directory
...
```
$ which python
/Users/jrminter/miniconda3/bin/python
```

The solution:

```
ln -s /Users/jrminter/miniconda3/bin/python /Users/jrminter/miniconda3/python.app/Contents/MacOS/python
```


# Remove PyCharm

From [blog.yinkos.com](http://blog.yinkos.com/2018/02/how-to-completely-uninstall-pycharm-on-macos/)
...
```
find ~/Library/ -iname "*pycharm*"
```

Verify that all of the results are in fact related to PyCharm and
not something else important you need to keep. Then, remove them
all using the command:
...
as needed. Add others to the list (and note the date with a comment below)
    ...
    # this last conda suggested by Curtis Rueden
    conda install ipywidgets beakerx
    pip install iversions
    ```
...
6. To remove an obsolete environment (`pweave`)

    ```
    conda env remove -n pweave
    ```
