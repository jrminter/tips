...
6. `conda update --all`
7. `conda install mysql-connector-python wheel`
8. Fix path in `startMIDB3.bat`
9. `cd %GIT_HOME%\OSImageAnalysis\py2\yml`
10. `conda env create -f py27.yml`
11. Go to hyperspy `git reset --hard; git checkout master`, install hyperspy from command line `(source) activate py27` and `python setup install`

**JRM Note:** For some reason on `ROCPW7ZC5C42` I cannot compile skimage without getting an error about not being able to import `io.h`. Works fine on `crunch`. Need to try `set` from both command prompts...
