...
title: "R Tips"
author: "J. R. Minter"
date: "Started: 2013-07-16, Last modified: 2022-03-26"
output:
  html_document:
...
1. How to get a PDF out of a RStudio Container

   - Check the box for the PDF in the **right** RStudio pane.

   - Click on the **gear** icon.

   - Click on **Export**

   - Press **Download** button on the "Export Files" dialog and press the 
   **Download** Box,

   - This should download the folder to your computer, typically your 
     **Downloads** folder.

   - You should be able to open the PDF and do what you want.


2. [R-Project](https://www.r-project.org/)    
   [R-4.1.3 macOS](https://mac.r-project.org/)            
   [Nightly builds for macOS](https://mac.r-project.org/#nightly)        
   
   **Note**: I was surprised this morning when no binaries were released
   for the R-4.1.1 were released this morning. Then I saw this tweet from
   Peter Dalgaard:
   
   > Peter Dalgaard @pdalgd (Tue Aug 10)   
   > #rstats 4.1.1 "Kick Things" (source version) has been released.
   > Binaries will follow in due course.
   
   This will be a good way to set my expectations for binary releases.
   The developers do not charge for their work and I am thankful for
   what they do. I want to follow this...
   
   [R-4.1.3 win beta build](https://cran.r-project.org/bin/windows/base/rtest.html)    
   [R-4.1.3.win](https://cran.r-project.org/bin/windows/base/index.html)

   > R version 4.1.1 (Kick Things) prerelease versions will appear starting
   > Saturday 2021-07-31. Final release is scheduled for 
   > **Tuesday 2021-08-10**.
   
3. Getting a PDF Document out of a RStudio Container

   A. Check the box for the PDF in the **right** RStudio Panel.
   
   B. Click under the `gear` icon.
   
   C. Click on `export`.
   
   D. Press Download button on the **Export Files** dialog and press the
      **Download**  Box.
      
   E. This should download the **PDF** to your computer, typically to the
      **Downloads** folder.
      
   F. You should be able to open the PDF and do what you want with it.
...
For a more detailed explanation on using BiocManager and its advanced usage, such as version switching, please refer to the BiocManager [vignette](https://cran.r-project.org/web/packages/BiocManager/vignettes/BiocManager.html).

# Key Rmarkdown resources

- [Getting Started with Rmarkdown](https://ourcodingclub.github.io/tutorials/rmarkdown/)

- [Griffiths Lab Rmarkdown](https://genviz.org/module-07-appendix/0007/03/01/introToRmarkdown/)

- [earthdatascience.org Intro to Rmd](https://www.earthdatascience.org/courses/earth-analytics/document-your-science/about-open-science-and-why-open-science-is-important/)


## Key packages

install.packages("ggplot2", "tidyverse", "tidyr", "shiny", "plotly", "knitr", "mlr3", "xgboost", "caret")
...
```
## From Peter Dalgard (2021-07-29)

## R-4.1.1 Release

> The release of 4.1.1 ("Kick Things") is scheduled for Tuesday 2021-08-10.
> Release candidate tarballs will be made available during the week up to
> the release.

> #rstats reminder: The countdown to 4.1.1 is imminent. (Beta versions Saturday,
> release candidates Tuesday, release Tue 10th.) So now rather than the 11th is
> a good time to check for remaining bugs.

## R Patched Builds

- R Patched **Nightly Builds** for
  [Windows](https://cran.r-project.org/bin/windows/base/rpatched.html).
...
- Rstudio [**Preview**](https://www.rstudio.com/products/rstudio/download/preview/) Release. For testing purposes...

## Rstudio Code

- [Run R in Visual Studio Code](https://www.infoworld.com/article/3625488/how-to-run-r-in-visual-studio-code.html)

## Configuring RStudio v1.4 Update

See [here](https://www.r-bloggers.com/2021/06/rstudio-v1-4-update-whats-new/)

```
.Renviron - Goes in Documents

WIN: PATH="${RTOOLS40_HOME}\usr\bin;${PATH}"


```

`usethis::use_blank_slate()`  sets your @rstudio preference to NEVER
save/restore `.RData` on exit/startup, which is a lifestyle endorsed by
many #rstats folks (including Jenny Bryan). I think it makes sense...


[raster data in R](https://www.neonscience.org/resources/learning-hub/tutorials/raster-data-r)

An **interesting** [rmarkdown](https://goessner.github.io/mdmath/publication.html) site.
Check it out...

Currently running RStudio 1.4.1717 on all my systems. This is a
"Preview Release". It seams pretty stable...

There is a helpful article [here](https://support.rstudio.com/hc/en-us/articles/200534577-Resetting-RStudio-Desktop-s-State)
on resetting RStudio's State.

Choose Primary CRAN repository as "Global (CDN) - RStudio". 

Some new developments are
[here](https://towardsdatascience.com/ten-awesome-recent-developments-in-r-6bfad46299a0).

## gfortran for R on macOS
...
> Xcode can be obtained from Apple AppStore and the Xcode developer page.
> Older versions are available in the "more" section of the Developer
> pages (Apple developer account necessary).
>
> **GNU Fortran compiler**    
> 
> Unfortunately Xcode doesn't contain a Fortran compiler, therefore you
> will have to install one. We recommend official GNU Fortran binaries
> from F.X.Coudert. CRAN High-Sierra builds currently use GNU Gortran
> 8.2 from that page (note that even though compiled for Mojave it is
...
>
> This is a copy of GNU Fortran 8.2 build from
> [here](https://github.com/fxcoudert/gfortran-for-macOS/releases)
>
> Make sure you add `/usr/local/gfortran/bin` to your PATH in order to
> use this compiler. This can be done, e.g. by
> 
> `export PATH=$PATH:/usr/local/gfortran/bin`
>
> NOTE: In order to retain compatibility with native R we recommend using
> above tools. Although it is possible to compile R using tools from
> other package managers such as Homebrew, MacPorts or Fink, such binaries
> are by definition incompatible with macOS native libraries and applications.
> If you choose one of those package managers, make sure you compile
> everything using those tools including R and all packages and libraries
> you intend to use.

## In RStudio

> TIF (today I found!)     
> Rstudio -> Tools -> Gobal Options -> General -> Advanced ->    
> 'DOUBLE-CLICK TO SELECT WORDS IN CONSOLE PANE' 

RStudio also added a new (2020-06-26) shortcut pull-down: 

!["pull with rebase"](inc/pull_with_rebase.png)

### R Function creation shortcut

`Hat Tip` Allison Horst:

> In a blank R chunk, if I enter a line like `apples + bananas`
> and highight that expression, and then hit
> `Control + Option + x` RStudio will provide a pop-up box
> requesting a function name and write a basic function for editing.

### Comment out R code shortcut

`Hat Tip` Erin Winslow:

Highlight the lines and type `CMS Shift C`

### Inset a code chunk shortcut

`Hat Tip` Juliet Verstaen

CMD + Option + i to create a code chunk




## Storing generated plots

I like to store generated plots as `.png` and `.jpg` for compatibility with
external software. Since these are generated `on the fly`, if one does not
need to track these files with version control, they can be be written to
a `png` folder and or a `jpg` folder. Add a `.gitkeep` file in the folder
so `git` will not track the file, Note that these files will be included in
embedded in your `.html` or `.pdf` files when you render the document. Those
files could be saved under version control and needed by the analysis.

## Rmarkdown Project Templates

I have a couple of zipped project templates
in Dropbox/R. This make life easier...

## Best Shortcuts

![Best R Shortcuts by Preet Chauhan](inc/best_R_shortcuts.png)

## Find commands in a package

From [Alex Recuenco](https://twitter.com/recuenco_alex/status/1256514483096186880)
on Twitter.

> My exploration starts writing `ggplot::` and then browsing the autocomplete...


## Helpful R OpenSci Packages

See the list [here](https://ropensci.org/packages/)

These helpful packages are on CRAN
...
> Some R packages require external libraries or software to build. `Rtools40`
> includes a package manager which you can use to install additional rtools
> packages.
> 
> There should be a shortcut to the Rtools terminal in your Start Menu.
>
> Run:
...
## Check your path from R

This may be helpful in diagnosing problems with `RTools` on Windows.
It came from Kevin Ushey [here](https://github.com/rstudio/rstudio/issues/3563).

On MacOS the command `writeLines(strsplit(Sys.getenv("PATH"), ":")[[1]])`
generates:
...
On Win 10 the command `writeLines(strsplit(Sys.getenv("PATH"), ";")[[1]])`
generates on jrmSimulationPC:

```
C:\Apps\R\R-3.6.3\bin\x64
C:\Apps\R\Rtools\bin
C:\Apps\ImageMagick
C:\windows\system32
C:\windows
...
C:\Apps\jdk-10\bin
C:\Apps\gs\gs9.50\bin
C:\Apps\local\path
.\Plugins
C:\Program Files\Gatan\Shared
C:\Apps\Brackets\command
C:\Program Files\Git\cmd
C:\Apps\texlive\2020\bin\win32
C:\Apps\Miniconda3
C:\Apps\Miniconda3\Scripts
C:\Users\johnr\AppData\Local\Microsoft\WindowsApps
```

and on jrmFastMacWin64
...
C:\Windows\System32\WindowsPowerShell\v1.0\
C:\Windows\System32\OpenSSH\
C:\Apps\texlive\2019\bin\win32
C:\Apps\local\path
C:\Program Files\Git\cmd
C:\Users\jrminter\AppData\Local\Microsoft\WindowsApps
```


## Problems installing packages from Github
...
Installation failed: Bad credentials
```
...
On my mac I seen to need to do this before each use of `install_github()`...

## Getting ready for R-4.0.0

I tested installation of R-4.0.0-beta and R-3.6.6 on my Parallels VM.

The RTools3.5 (latest for the R-3 branch) has a choice to add it to
the path.

No matter what I tried, I could not get R-4.0.0 to find the RTools-4.0
install.



## Easily add a code section in RStudio

![Insert a code chunk from the RStudio GUI](inc/R-Studio_insert_code_chunk.png)

## Easily handle cards, dice, and coins

Note: the first two don't render correctly on windows

From Rasmus Bååth @rabaath on
[twitter](https://twitter.com/rabaath/status/1251118874453516289) 2020-04-17.

```{r handleCards, comment=NA}
suits = c("♣", "♦", "♥", "♠")
values = c(2:10, "J", "Q", "K", "A")
cards = paste0(rep(values, each=4), suits)
cat(sample(cards))
```

and dice on
[twitter](https://twitter.com/rabaath/status/1251117100791447557) 2020-04-17.


```{r rollTheDice, comment=NA}
dice = c("⚀", "⚁", "⚂", "⚃", "⚄", "⚅")
cat(sample(dice, 200, replace = TRUE))
```

and my adaptation of his coin-flips (I found the images annoying)

```{r tossCoin, comment=NA}
head_tails = c("H", "T")
cat(sample(head_tails, 20, replace = TRUE))
```

## Use the `glue` package to process strings


From [here](https://github.com/tidyverse/glue)
...
Find it [here](https://holtzy.github.io/Pimp-my-rmd/).
...
```{r testHH, echo=TRUE, message=FALSE, comment=NA}
library(here)
library(lubridate)
...
Finally, the upcoming Debian “Buster” release will ship with
...
[João Neto](http://www.di.fc.ul.pt/~jpn/r/) has some really nice
...
for that — we may ask GNU make to allow more than one job to be submitted
