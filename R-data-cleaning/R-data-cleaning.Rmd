...
author: "J. R. Minter"
date: "Started: 2019-04-30, Last modified: 2019-30-29"
output:
  html_document:
...
library(janitor)
library(dplyr) # for pipe (%>%) operaror
...
If tou want to rearrange a group of columns to the left of the dataframe:
...
If you want to move them to a specific place, you can specify which they should
follow or precede. In this example both of the following have the same effect:

```{r rearrangeTwo}
mtcars %>%
  relocate(vs:gear, .before = cyl) %>%
  head(3)

mtcars %>%
  relocate(vs:gear, .after = mpg) %>%
  head(3)
