...
author: "J. R. Minter"
date: 'Started: 2015-05-19, Last modified: 2021-04-05'
output:
  pdf_document:
    toc: yes
    toc_depth: '3'
...
# DTSA-II Prerelease installers

They are [here](https://drive.google.com/file/d/17YAXRYkQ9vAQ509AwP5eZagdGfShX-dE/view?usp=sharing).

Current full path is shown below:

```
https://drive.google.com/file/d/17YAXRYkQ9vAQ509AwP5eZagdGfShX-dE/view?usp=sharing
```

I have many installers in `D:\_installers\dtsa2-installers`.
These tend to be named `dtsa2-prerelease`. One can obtain the date of
production by running the following code in a terminal

```
java -jar dtsa2-prerelease.jar
```

As of 2021-04-05, it seems like the latest installer is 2021-03-19.

A window will open for the install and it will have a date. One can
quit the installer and append the date to the `*.jar` file. I store
these on my Win10 box...

# Helpful conversion of an image to SVG

On 2021-07-08 Oldrich wrote on the Probe Software Forum:

> Suddenly I cannot save the spectra in SVG. Now, there are only options
> to save them in bitmap format (TIFF, JPEG, PNG, BMP). No possibility
> to save the spectrum in the vector graphic 
> (Debian Buster 5.10.0-0.bpo.5-amd64, DTSAII,
> Microscopium 2021-01-06 revision). The same on the old box running
> Win7 and the identical version of DTSAII.

There is an online conversion program at:

[picsvg.com](https://picsvg.com/)

I'm not sure how well it will work...

[Here](https://picsvg.com/).

# Helpful DTSA-II Videos
...
maybe relative theoretical intensity of M lines for REE are a bit too far from correct in the database of DTSA; In particular If You will check the height of La M3-N1 line marker (in the attachment) is quite strongly undershot; and maybe due to to little intensity it is modeled to be completely absorbed.
