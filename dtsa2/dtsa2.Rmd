...
author: "J. R. Minter"
date: "Started: 2015-05-19, Last modified: 2018-10-02"
output:
  knitr:::html_vignette:
...
- The mouse wheel zooms in and out on intensity. Can also click and drag
by the counts on the left side.
...
from this menu. Can select how lines are labeled. IUPAC is preferrred
or element abbreviation for an uncluttered look.

- **Copy as bitmap** produces a high resolution bitmap. Scaling is a bit
counterintuative. Lettering size is a function of size on the screen.
...
fractions.    
    ...
    capitalize the forst letter of each element's abbreviation or you
...
- There are two "Composition" values. There is the accepted or standard
value that we enter through the material editor and the measured 
composition from spectra.

- Spectra list also has a context-sensitive menu accessed by right click

- Report tab has summary of the experiment. Because it is HTML it is
searchable using the OS. The Report menu has a "Report Note" button that
lets you add comments or notes to the report. You can also add the 
spectrum display to the report.

- The command line is useful for repetitive tasks. The `tabulate()`
function is helpful. For example, one might seclect a range of values
and enter
...
fits.
    ...
    qunatitative analysis is to choose a standard similar in composition
    ...
    In this case we choose Benitioite. The relative intensity in the
