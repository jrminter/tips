...
Probe for EPMA maintains a standard satabase that is very convenient to
use when the standards have been set up. I found setting them up a bit 
confusing at first. CalcZAF makes use of these for the computaions and
...
In one of our labs in the Lehigh Quantitative Microanalysis course we wanted
to create three standards for Ir~X~Si~Y~ intermetallics and to use one that
had been verified by wet chemistry as a standard to analyze an unknowns to
see if we could properly identify the unknown. In this case, we will set up
the standard for the **Ir~3~Si~5~** intermetallic that will be used as 
a standard in our computations.

We create the standard from the **Standard.exe** program. When we start
that proigram we need to choose a standard file. In this case we use the
default, **standard.mdb**. The top menu bar has a **Standard** button that
displays the following menu. 

This get's us a dialog box that we can fill in. The figure below shows a
...
We repeat the process for Si and then I computed an estimate of the density
that was in the completed dialog above.

Standard numbering is systematic for Probe for EPMA. For our use with CalcZAF
we can assign non-assigned numbers. I'll figure out the convention later...
...
So we can set up our analysis of our unkown as follows:

First add the information for the unknown for **both** ir and Si and assign the standard:
...
```
CalcZAF Sample () at 40 degrees and 15 keV

STANDARD PARAMETERS (TOA= 40):

 ELEMENT  STDNUM STDCONC STDKFAC   Z-BAR  ABSCOR  FLUCOR  ZEDCOR  ZAFCOR                  
   Ir La    2843  78.245   .6800 63.2944   .9937  1.0000  1.1580  1.1506
   Si Ka    2843  21.755   .2136 63.2944  1.3168   .9924   .7793  1.0185

 ELEMENT STP-POW BKS-COR   F(x)e   F(x)s      Eo      Ec   Eo/Ec
   Ir La  1.1893   .9737   .9735   .9797   15.00 11.2150  1.3375
   Si Ka   .6033  1.2918   .9056   .6877   15.00  1.8390  8.1566

SAMPLE: 0, TOA: 40, ITERATIONS: 3, Z-BAR: 66.20833

 ELEMENT  ABSCOR  FLUCOR  ZEDCOR  ZAFCOR STP-POW BKS-COR   F(x)u      Ec   Eo/Ec    MACs uZAF/sZAF
   Ir la   .9948  1.0000  1.1242  1.1183  1.1486   .9787   .9786 11.2150  1.3375 120.085  .9719211
   Si ka  1.3418   .9919   .7678  1.0219   .5812  1.3211   .6749  1.8390  8.1566 1267.54  1.003356

 ELEMENT   K-RAW K-VALUE ELEMWT% OXIDWT% ATOMIC% FORMULA KILOVOL                                        
   Ir la 1.07200  .72900  81.523   -----  41.416   3.313   15.00                                        
   Si ka  .77200  .16490  16.851   -----  58.584   4.687   15.00                                        
   TOTAL:                 98.375   ----- 100.000   8.000

```

The totals are not too bad... I don't rememeber f(chi) being that bad...
...
![**Note the large number of default trajectories (2.0e9) and the long simulation time (1.0e5 sec = 27.8 hrs!)** You probably want to change these to something like 2.0e4 trajectories and 3600 sec (1 hr). Do this **before!!** you press **Create PENEPMA input file.**](./img/penepma-default-input-par.png)
...
And this is what I obtained. Note that if one wanted to compute K-ratios from spectra, it might be worth computing more trajectories... It would also be nice to be able to export these to **.msa** spectrum files to plot in DTSA-II with markers and make publication quality graphics with gnuplot... I think Probe for EPMA uses Surfer... 
