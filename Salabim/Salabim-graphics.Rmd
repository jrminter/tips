...
By [Tim Salabim](https://github.com/tim-salabim/metvurst/tree/master/markdown)

This tutorial was developed as part of a one-day workshop held within the Ecosystem Informatics PhD-programme at the Philipps University of Marburg. The contents of this tutorial are published under the creative commons license ['Attribution-ShareAlike CC BY-SA'](http://creativecommons.org/licenses/by-sa/3.0/).
...
Another resource for creating visualizations using R, more focused on climatological/atmospheric applications can be found at http://metvurst.wordpress.com
...
* become familiar with the two main packages for highly flexible data visualization in R - lattice & ggplot2 (biased towards lattice I have to admit)
* learn how to modify the default options of these packages in order to really get what we want 
* learn even more flexibility using the grid package to create visualizations comprising multiple plots on one page and manipulate existing plots to suit our needs
...
Now the new ```diamonds.cheap``` subset is a reduced data set of the original ```diamonds``` data set only having `r nrow(diamonds.cheap)` entries instead of the original `r nrow(diamonds)` entries.
...
* `diamonds.premium.and.cheap` has `r nrow(diamonds.premium.and.cheap)` rows, while
* `diamonds.premium.or.cheap` has `r nrow(diamonds.premium.or.cheap)` rows
...
Right, so now that we have established some principles about color representation, we can go ahead and use this to show the density of the points in our scatter plot.
...
It should not go unnoticed that there is a panel function in ```lattice``` that does this for you. The function is called ```panel.smoothScatter()``` and unless we need to specify a custom color palette, this is more than sufficient.
...
And, as we've already seen, the faceting is also just one more line...
...
But, it is not too complicated. The equation for this adjustment is rather straight forward, we simply take the square root of the counts for each color and divide it by the overall number of observations. Then we standardize this relative to the maximum of this calculation. As a final step, we need to break this down to each of the panels of the plot. This is the toughest part of it. I won't go into any detail here, but the ```llply``` pat of the following code chunk is basically the equivalent of what is going on behind the scenes of ```lattice``` (though ```lattice``` most likely does not use ```llply```).
...
Afterwards we navigated into this viewport - `pushViewport(our.first.vp)` and then we have drawn a rectangle that fills the entire viewport and filled it in pink color - `grid.rect(gp = gpar(fill = "pink"))`
...
```{r grid multiple, tidy = FALSE, fig.cap = fn("using the grid package to place multiple plots on one page"), fig.align = 'center'}
### clear plot area
grid.newpage()
...
```{r grid manipulate, tidy = FALSE, fig.width = 10, fig.height = 7, fig.cap = fn("using the grid package to add a color key to an existing lattice plot object"), fig.align = 'center'}
grid.newpage()
#grid.rect()
print(l.sc, newpage = FALSE)
#grid.rect()
...
Not too complicated, ey. And, in comparison to the `ggplot2` version, we are utilizing the available space a bit more efficiently. Though, obviously, we could also manipulate the `ggplot2` density scatter plot (or any other plot) in a similar manner. 
...
```{r saving png, eval = FALSE}
png("test.png", width = 14.7, height = 18.8, 
    units = "cm", res = 600, pointsize = 12)
...
* we heave also seen how to produce panel/faceted plots providing some statistic for each subset of the data
...
The last part of this tutorial is concerned with a more specific topic, the use of spatial data in R for creating maps. As this workshop is aimed at environmental sciences students, I have decided to include at least a tiny little introduction into the use of spatial data with focus on creating maps (after all this workshop is about visualization, not analysis).
...
Then, the R binding to the `gdal` libraries, called `rgdal`, should install just like any other package. The other package that we will need to install for any spatial task in R is `sp` (for spatial). This is the center of the spatial universe in R and any other package for spatial analysis is dependent on it. A final package that we will need here is `raster` which is, as the name already implies, focused towards the spatial analysis of raster data. 

### 5.1. Polygon data

It may come as a surprise for many people, but R can, with the aid of a few packages, be used as a full-blown GIS system. Especially, the visualization of maps is rather straight forward.

Using the functionality provided by ```sp```, there are several hundred packages that provide extended possibilities for the analysis and visualization of spatial data. A comprehensive list of available packages can be found [here](http://cran.r-project.org/web/views/Spatial.html).

One nice thing about `sp` is that (nearly) all plotting routines are using the ```lattice``` package, so that they are highly customizable (and most of the things we learned here are directly applicable to spatial representations using `sp` or related packages).

As an example, let's try to produce a graphic that uses color to represent a certain continuous variable mapped to each of the federal states of Germany. In our case here, we create a random variable based on a Poisson distribution and assume it represents the number of breweries in each German federal state.
...
```{r spatial polygon data, tidy = FALSE, message = FALSE}
...
```{r plotting polygons, tidy = FALSE, fig.width=15, fig.height=10, fig.cap = fn("using packages sp, raster, RColorBrewer, and grid to create a multiple map plot"), fig.align = 'center'}
...
3. we need to divide the temperature (which are in degrees Celsius) by a factor of 10 (in order to save space, these data are stored as integers and hence multiplied by 10)
4. we download the outline of Germany from `gadm`
5. to highlight the problems related to color representation we already touched upon in section 2.1.1. we plot 5 different representations of the same data on one page (similar to the polygon data from section 5.1.)

```{r spatial raster data, tidy = FALSE, message = FALSE}
# Tmean <- getData('worldclim', var = 'tmean', res = 5)
load('./dat/Tmean.RData')
ext <- extent(gadm)
...
```{r plotting raster data default, tidy = FALSE, fig.width=10, fig.height=15, fig.cap = fn("a basic raster plot using raster package"), fig.align = 'center'}
Tplot <- spplot(Tmean.ger, zcol = "tmean8")
print(Tplot)
```

So, let's manipulate the colors a little...

```{r plotting raster data coladjustment, tidy = FALSE, fig.width=10, fig.height=15, fig.cap = fn("a basic raster plot using raster package"), fig.align = 'center'}
...
Now, we can plot the different versions and compare the usefulness of the different color palettes.

```{r plotting multiple rasters, tidy = FALSE, fig.width=20, fig.height=15, fig.cap = fn("using packages sp, raster, RColorBrewer, and grid to create a multiple raster map plot"), fig.align = 'center'}
grid.newpage()
...
Again, the `hcl` based color palette (top right panel) seems to be the most suitable, as data distortion is minimized and at the same time it provides multiple hues which help to distinguish the temperatures in more detail than e.g. the mono-hue palettes (bottom panels). The Matlab jet color palette - here `tim.colors()` clearly shows the worst distortion by creating several artificial cold 'islands/pools', especially in the German low-mountain-ranges (in the middle of the country). The colorbrewer `Spectral` palette is acceptable in that it does not distort the data very much, yet it is technically a diverging palette which is not optimal for representing data of sequential nature.

Ok, so now we have a rather comprehensive, though far from complete (if that is even possible) set of tools for visualizing our data using classical statistical plots as well as spatially mapped. I admit, that the spatial part is very limited here, but that may be part of another tutorial in the future. 
