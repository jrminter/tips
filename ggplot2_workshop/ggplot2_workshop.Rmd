...
author:
  - Thomas Lin Pedersen (producer)
  - John Minter (editor)
date: "Original: 2015-04-04, Last modified: 2020-04-24"
output:
  html_document:
...
Thomas Lin Pedersen created a very helpful workshop consisting of two videos
on YouTube.
...
This document comes with a list of required packages. Let's also add a boolean
variable to let us control running animations. They take a **long time** and
do not knit into the HTML or display in the RStudio Viewer.... 

I (John Minter) created separate R scripts to run the four animations. 
...
# Animations do not show up in the viewer
# The Boolean variable does not run thesw while knitting.
runAnim <- FALSE
start_time <- Sys.time()
library(ggplot2)
```
...
Was 40.401 sec with animations off on `jrmFastMac` with animations off.

Was 3.95 min kniting with animations on and they don't show up in the viewer.
I should extract them... For now, leave the Boolean variable OFF.
