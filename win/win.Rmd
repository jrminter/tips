...
# How to take Screenshots in Widows 11

1. Use **Fn PrtSc (Print Screen)** - under the **Insert** key and **Paste**.
   Since well before Windows 11 you have been able to snap a screenshot with the
   press of a single key: **PrtSc**.
   
2. Use the **PrtSc** Key with OneDrive.

   
3. Use the **Windows Key** - **PrtSc Keyboard Shortcut**. ...

4. Use the **Snipping Tool**

5. Use the **GameBar** (I do not have the GameBar on my Keyboard...)



# How to back up Windows 11

I do this a lot. I want it **easily visible**...
...
# How to open a command prompt in Windows 11

1. Press the **Windows** + **R** keys from the keyboard to open the **Run** dialog box.

2. In the **Run** box type **CMD** and then press `Ctrl` + `Shift` + `Enter` 
from the keyboard to run the command prompt as an administrator.

3. You should see a dialog box to enter the command ('CMD`).

4.  If **User Account Control** appears, select the `YES` button.
the keyboard to  run the command prompt as an  

# Some useful quick tips for Windows 10/11

**Alt-F4** prompts for a **restart**

**Scans** are in `C:\Users\my-login\Dropbox\scans`
