...
title: "Octave Tips"
author: "J. R. Minter"
date: 'Started: 2019-03-21, Last modified: 2019-04-13'
output:
  html_document:
...
1. `ones(n,m)` - Generates a matix `n x m` of 1's

2. `zeros(n,m)` - Generates a matix `n x m` of 0's

3. `eye(n) or eye(n,m)` Generates an identity matix

4. `rand(n,m)` Generates a matix `n x m` of randomly distributed elements
...
- length(m) - returns the longest dimensio
...
The two control statements in Octave are `if` statments ans `repetitions`
