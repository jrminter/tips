...
Rmarkdown uses YAML to encode information for later steps in the document
generation process. Each document begins with a YAML header. 
This is an example of a YAML header for an analysis
...
abstract: |
  ...
  morpholgy appeared to be a mixture of small needles and small equiaxial
  ...
  of 3-4\%. The larger clumps appeared to have more C than the that from the
  evaporated film on the Si chip.
output:
  knitr:::html_vignette:
    css: ../theme/jm-mod-vignette.css
    number_sections: yes
...
We can get fine contol of image size like this:

1. Start by adding thse packages to the first R chunk
